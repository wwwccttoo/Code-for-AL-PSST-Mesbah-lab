...
% load initial data
%X0 = [rand(200,1)*50+20,rand(200,1)*0.5,rand(200,1)*5+2,rand(200,1)*6.5+0.5];
X0 = [55, 0.05, 12.0, 5.0];

bounds=[20 70;0 0.2095;2 15;0.5 7];
initial_bud_c=15;
dim = size(bounds,1);

%% Defining a struct called "problem" for setting the objective and constraint functions & bounds
problem.f='ftir_NN';
problem.f_parameters=[50];
problem.c={'ftir_NN'}; % a struct to enable multiple constraints
problem.c_parameters={[50]};
...
coevalF = @(X) funcf([X(1),X(2),X(3),X(4)],problem.f_parameters);

coevalC = cell(length(problem.c),1);
for i=1:length(problem.c)
    func=str2func(problem.c{i});
    coevalC{i}= @(X) func([X(1),X(2),X(3),X(4)],problem.c_parameters{i});
    clear func
end

problem.coevalF = coevalF;
problem.coevalC = coevalC;
...
for i=1:num
    ...
    initial_samples = X0;%X0((i*initial_num)-1:(i*initial_num),:);
    ...
    % Run ADMMBO
    [Samples{i},Values{i},best_point{i},best_F{i},best_C{i}] = ADMMBO(problem,opt);
    
    % Report the results
    feasible_sum = 0;
    for k =1:length(problem.c)
        feasible_sum = feasible_sum + (best_C{i}(k) <= tolC);     
    end
...
%exit
resss = [];
for i=1:length(Samples{1,1})
    resss = [resss;ftir_NN(Samples{1,1}(i,:),50)];
end
