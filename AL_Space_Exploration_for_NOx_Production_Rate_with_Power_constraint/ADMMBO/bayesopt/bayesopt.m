function [minsample,minvalue,botrace] = bayesopt(F,opt)
	...
	if isfield(opt,'grid')
        ...
        %to make this random
        %randinter = randi([0 10000],1,1);
    	%hyper_grid = sobol(1+randinter:opt.grid_size+randinter,:);
        hyper_grid = rand(opt.grid_size,opt.dims);
	...
	if isfield(opt,'resume_trace') && opt.resume_trace && exist(opt.trace_file,'file')
		...
		clear botrace
    elseif isfield(opt,'AL_evals')
        samples = scale_point(opt.samples,opt.mins,opt.maxes);
		values = opt.AL_evals;
		times = [];
	else
		samples = [];
    ...
    for i = i_start:opt.max_iters-2,
		...
		if PAR_JOBS <= 1,
            if ~DO_COEVAL
                ...
                [hyper_cand,hidx,aq_val] = get_next_cand(samples,values(:,1),hyper_grid,opt,DO_CBO,con_values,OPT_EI,EI_BURN);
            end
		else
            ...
            if ~DO_COEVAL
                ...
                [mu_obj,sigma2_obj] = get_posterior(samples,values(:,1),hyper_grid,opt,-1);
            end
			...
			if ~DO_CBO,
                ...
                if ~DO_COEVAL % useless and could be wrong
                    best = min(values(which_feas));
                    if isempty(best), best = max(values)+999; end
                else
                    best = min(values(which_feas,1));
                    if isempty(best), best = max(values(:,1))+999; end
                end
				ei = compute_ei(best,mu_obj,sigma2_obj);
			...
			for j = 2:PAR_JOBS,
                ...
                if ~DO_COEVAL
                    ...
                    [mu_f,sigma2_f] = get_posterior(samples,values(:,1),hyper_cands,opt,-1);
                end
				...
				parfor k = 1:MC_ITERS, % MCMC loop to get EI marginalized over fantasy candidates
					...
					if ~DO_CBO,
                        if ~DO_COEVAL
                            ...
                            [mu,sigma2] = get_posterior([samples;hyper_cands],[values(:,1);fant_y],hyper_grid,opt,-1);
                            best = min([values(:,1);fant_y]);
                        end
                        ...
                        if ~DO_COEVAL
                            ...
                            [mu_obj,sigma2_obj] = get_posterior([samples;hyper_cands],[values(:,1);fant_y],hyper_grid,opt,-1);
                        end
						fant_cvals = [];
                        ...
                        if ~DO_COEVAL % useless and could be wrong
                            ...
                            best = min([values(which_feas,1);fant_y(which_fant_feas)]);
                            if isempty(best), best = max([values(:,1);fant_y])+999; end
                        end
		...
		if ~DO_CBO,
			...
			if PAR_JOBS <= 1,
				...
				if ~DO_COEVAL %here is useless and could be wrong
                    values(end+1) = value;
                else
                    values(end+1,:) = value;
                end
		...
		if PAR_JOBS <= 1,
	        if ~DO_CBO,
                ...
                if ~DO_COEVAL %useless and could be wrong
                    ...
                    fprintf(', value = %f, feasible = %d, overall min = %f\n',value(1),all(con_value<=opt.lt_const),min(values(which_feas,1)));                    
                end
			end
		else
			if ~DO_CBO,
				...
				which_feas = all(bsxfun(@le,con_values,opt.lt_const),2);
                if ~DO_COEVAL %useless and could also be wrong
                    fprintf('Overall min = %f\n\n',min(values(which_feas)));
                else
                    fprintf('Overall min = %f\n\n',min(values(which_feas,1)));
                end
			end
		end
